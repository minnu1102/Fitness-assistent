...
# ---------------------- INPUT FORM ----------------------
with st.form(key="fitness_form_unique"):
    name = st.text_input("Name", placeholder="Enter your name")
    age = st.number_input("Age", min_value=10, max_value=100, value=21)
    weight = st.number_input("Weight (kg)", min_value=20, max_value=250, value=60)
    ...
    equipment = st.text_input("Equipment Available", placeholder="e.g., dumbbells, yoga mat, none")
...
for key in agents:
    try:
        ...
        st.warning(f"⚠️ Could not start {key} agent: {e}")
...
if submitted:
    if not name.strip():
        ...
        with st.spinner("Generating your personalized AI fitness plan..."):
            ...
            def run_assess(): results['assess'] = safe_assess(agents['assessment'], user_info)
            def run_nutri(): results['nutri'] = safe_nutrition(agents['nutrition'], name, age, weight, height, f"{goal} ({bmi_status})")
            def run_exer(): results['exer'] = safe_exercise(agents['exercise'], userid, name, age, weight, height, f"{goal} for {fitness_focus}", days, equipment)
            def run_motiv(): results['motiv'] = safe_motivation(agents['motivation'], user_info)
        ...
        st.markdown(f"<div class='plan-text'>{results.get('nutri', ' No nutrition plan available.')}</div>", unsafe_allow_html=True)

        st.markdown("<div class='section-title'>🏋️ Workout Plan</div>", unsafe_allow_html=True)
        exer_text = results.get('exer', ' No workout plan available.')
